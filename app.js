(() => {
  ...
  const state = {
    ...
    ui: { filterSection: "", filterCategory: "", search: "", topMode: false },
    catalog: [], // {id, name, category, sections[], createdAt}
    order: { items: [] },
    stats: { usage: {}, catalogUrl: "", catalogUpdatedAt: "" } // {id, name, category, qty, updatedAt, by}
  };
...
<script>window.onload=()=>{ setTimeout(()=>window.print(), 150); };</script>
  ...
  function printExportHurt(){
    const out = buildExportText();
    const title = `${out.title} – ${out.date}`;
    const meta = out.meta || "";
    ...
    for(const ln0 of lines){
      ...
      if(m){
        currentCat = m[1].trim();
        typed.push({k:"cat", text: currentCat});
        continue;
      }
      if(ln.trim().startsWith("- ")){
        typed.push({k:"item", text: ln.trim().slice(2), cat: currentCat});
        continue;
      }
    ...
    const cost = (k)=> (k==="cat" ? 2 : (k==="item" ? 1 : 1));
    ...
    const push = (k,text,catForCont)=>{
      const c = cost(k);
      if(used[col] + c > LINES_PER_COL){
        startNewCol(catForCont && k!=="cat" ? catForCont : "");
      }
    ...
    win.document.open();
    win.document.write(`<!doctype html><html><head><title>${escapeHtml(title)}</title>${style}</head><body>${pagesHtml}<script>window.onload=()=>{window.print(); setTimeout(()=>window.close(), 300);};</script></body></html>`);
    win.document.close();
  }
...
$("btnBack").addEventListener("click", () => showPanel("panelOrder"));
  ...
  async function importCatalogFromGoogleSheets(){
    try{
      ...
      const res = await fetch(state.settings.catalogCsvUrl, { cache: "no-store" });
      ...
      for(let i=1;i<rows.length;i++){
        ...
        const sections = idxSections !== -1 ?
          String(r[idxSections] || "").split(";").map(s => capFirst(norm(s))).filter(Boolean) :
          [];
        out.push({ id: uid(), name, category, sections });
      }


  function refreshCatalog(){
    const url = (state.settings.catalogCsvUrl || state.stats.catalogUrl || "").trim();
    if(!url){
      toast("Brak linku CSV. Najpierw: Wczytaj z Google Sheets.");
      return;
    }
    importCatalogFromGoogleSheetsSilent(url);
  }


async function importCatalogFromGoogleSheetsSilent(url){
    try{
      state.settings.catalogCsvUrl = url.trim();
    save();
      ...
      if(out.length === 0) throw new Error("Nie znaleziono żadnych produktów w CSV");
      state.catalog = out;
      save();
      renderAll();
      toast("Wczytano bazę produktów");
